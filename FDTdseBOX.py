...
plt.plot(x, VBox(V0, a, x), "k--", label=r"$V(x) = \frac{1}{2}m\omega^2 (x-5)^2$ (x0.01)")
...
def psi_t(t, psi):
    for i in range(500):
        return -1j * (- 0.5 * hbar / m * D2.dot(psi) + VBox(V0, a, x)[i] / hbar * psi)
...
plt.plot(x, VBox(V0, a, x), "k--", label=r"$V(x) = \frac{1}{2}m\omega^2 (x-5)^2$ (x0.01)")   # Plot Potential
plt.legend(loc=1, fontsize=8, fancybox=False)
