...
def barrier(x,V0):
    ...
    while valid == 1:
        form = input("""Specify the form of potential to be computed:\n
                   1. Forward step potential [WORKING]\n
                   2. Backward step potential [WORKING]\n
                   3. Barrier potential ($v>v_0$) [BROKEN]\n
                   4. Barrier potential ($v<v_0$) [WORKING]\n
                   5. Infinite potential well [BROKEN]\n
                   NOTE: Please enter '1', '2', '3', '4', '5'.\n""")
                   
        if form == "1":
            start = float(input('At what value x does the step begin?\n'))

            for i, element in enumerate(x):
                
                V_x[float(x[i]) > start] = V0
        
            valid = 2
            break
            
        if form == "2":
            end = float(input('Input where the step ends:\n'))
            
            for i, element in enumerate(x):
                V_x[float(x[i]) < end] = V0
        ...
        if form == "3":
            print("\nThis form is a work in progress, rerouting to the selection screen......\n")
...
V0 = float(input("""
                 Welcome to this TDSE solution program!!!!
                 
                 WARNING: THIS PROGRAM USES A HEAVY AMOUNT OF RAM (10GB +)
                 
                 Input potential value [unnormalised]:\n"""))
...
#print("this is x",x) ### prints everything in the x array

barrier(x,V0)

#for i, element in enumerate(V_x):
#    print(V_x[i]) #### prints all the elements of the V_x array
    
#print("max V_x",np.max(V_x)) #### prints the max value of V_x array to see if the program is running correctly
